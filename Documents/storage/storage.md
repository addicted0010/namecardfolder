...
graph LR
    ...
    E -->|非名片| G[抛出 422 错误]
    F --> H[从原图全尺寸裁剪]
    H --> I[压缩≤1000px quality=72]
    I --> J[LLM 朝向检测]
    J -->|需旋转| K[Sharp 旋转图片]
    J -->|正向| L[删除初始文件+上传最终文件]
    K --> L
```
...
6. **全尺寸裁剪**：对原始图片进行精确裁剪
7. **压缩**：缩放到 ≤1000px，JPEG quality=72
8. **朝向检测**：混合策略——先用图片尺寸判断（竖图必定需旋转），再用 LLM 验证旋转方向（90° or 270°）。横图则用 LLM 检测是否倒置（180°）
9. **旋转校正**：如需旋转，使用 Sharp 顺时针旋转对应角度
10. **存储替换**：删除初始完整图 → 上传最终处理后图片
