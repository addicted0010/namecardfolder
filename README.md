...
- **每日免费额度保护**：普通账号每天自动获得 100 credit；单面名片消耗 1 credit，双面名片消耗 2 credit，管理员账号不受限制。
...
4. 点击完成时会按图片面数扣除今日 credit，上传完成后 AI 会在后台识别；你可以继续添加下一张名片，不需要等待。
...
| `CREDIT_TIME_ZONE` | 每日 credit 重置时区，默认 `Asia/Tokyo` |
